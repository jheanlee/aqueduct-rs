...
export class Migration20260806140916 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `create table "tunnel_users" ("id" varchar(21) not null, "username" text not null, "token" text not null, "hashed_password" text not null, "label" text[] not null, "last_login" timestamp not null, "administrator" boolean not null, constraint "tunnel_users_pkey" primary key ("id"));`,
    );
    this.addSql(
    ...
    this.addSql(
      `create table "tunnel_sessions" ("id" bigserial primary key, "user_id" varchar(21) null, "bucket_start" timestamp not null, "tunnel_client" inet not null, "inbound" bigint not null, "outbound" bigint not null, "external_connection_count" bigint not null);`,
    );
    this.addSql(
