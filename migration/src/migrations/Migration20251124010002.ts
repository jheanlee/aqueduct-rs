...
export class Migration20251124010002 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table "ip_blacklist" ("id" serial primary key, "subnet" inet not null, "comment" text not null);`);

    this.addSql(`create table "ip_whitelist" ("id" serial primary key, "subnet" inet not null, "comment" text not null);`);

    this.addSql(`create table "settings" ("key" text not null, "value" text not null, constraint "settings_pkey" primary key ("key"));`);

    this.addSql(`create table "statistics" ("id" varchar(21) not null, "inbound" bigint not null, "outbound" bigint not null, constraint "statistics_pkey" primary key ("id"));`);
