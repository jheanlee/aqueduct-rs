...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::DatabaseNotInitialized => write!(
                f,
                "Database not initialized. Please apply migration with the subcommand `migrate up`"
            ),
            ...
            Self::UnsupportedMigrationVersion => write!(
                f,
                "Unsupported database version. Please upgrade by applying migration with the subcommand `migrate up`"
            ),
        }
    }
}
