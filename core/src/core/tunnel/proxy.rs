...
pub async fn tunnel_client_proxy_control(
  ...
  if let Some(tcp_listener) = tcp_listener {
    ...
    log(Level::Info, format!("Tunnel port {} assigned to client {}, started listening", port ,tunnel_client.addr.to_string()).as_str(), "core::tunnel::proxy::tunnel_client_proxy_control").await;

    loop {
      select! {
        result = tcp_listener.accept() => {
          match result {
            Ok((mut external_client_stream, external_client_addr)) => {
              //  generate id
              let id = nanoid!();
